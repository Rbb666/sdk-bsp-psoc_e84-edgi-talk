...
RETRO_GO_LINKER_SCRIPT = 'link.ld'
try:
    with open('rtconfig.h', 'r', encoding='utf-8') as config_file:
        if '#define BSP_RETRO_GO_CORE_ITCM' in config_file.read():
            RETRO_GO_LINKER_SCRIPT = 'link_itcm.ld'
except OSError:
    pass
...
if PLATFORM == 'gcc':
    ...
    LFLAGS = DEVICE + ' -flto -Wl,--gc-sections,-Map=rtthread.map,-cref,-u,Reset_Handler -T board/linker_scripts/' + RETRO_GO_LINKER_SCRIPT + ' -flto'
