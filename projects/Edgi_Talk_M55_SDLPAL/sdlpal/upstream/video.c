...
#if defined(PAL_PSOC_DIRECT_INDEXED)
   pal_memory_clear_framebuffers();
   gpScreen = SDL_CreateRGBSurfaceFrom(pal_framebuffer_primary,
      320, 200, 8, 320, 0, 0, 0, 0);
   gpScreenBak = SDL_CreateRGBSurfaceFrom(pal_framebuffer_backup,
      320, 200, 8, 320, 0, 0, 0, 0);
...
--*/
{
#if defined(PAL_PSOC_DIRECT_INDEXED) || defined(PAL_EXTREME_TWO_SCREENS)
   (void)lpRect;
...
#elif defined(PAL_PSOC_DIRECT_INDEXED)
	sprintf(filename, "pal-%010lu.bmp", (unsigned long)SDL_GetTicks());
#elif !defined( GEKKO )
	struct timeval tv;
	struct tm *ptm;
